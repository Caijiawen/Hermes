...
if resume:
    ...
    model = {}
    model['W1_policy'] = np.random.randn(
        H, D) / np.sqrt(D)  # "Xavier" initialization
    model['b1_policy'] = np.random.randn(H) / np.sqrt(4*H)
    model['W2_policy'] = np.random.randn(H) / np.sqrt(H)
    model['b2_policy'] = 0.0
    model['W1_value'] = np.random.randn(
        H, D) / np.sqrt(D)  # "Xavier" initialization
        ...
        if round_number % batch_size == 0:
            for k, v in model.items():
                ...
                if rmsprop:
                    ...
                    momentum[k] = mom_rate * momentum[k] + \
                ...
                if 'value' in k:
                    model_target[k] = mom_rate * \
                        model_target[k] + (1-mom_rate) * model[k]

        print(('round %d game finished, reward: %f' %
               (round_number, reward)) + ('' if reward == -1 else ' !!!!!!!!'))
